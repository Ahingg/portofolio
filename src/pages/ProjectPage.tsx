...
export default function ProjectPage() {
    ...
    return (
        <>
            <div className="py-10">
                ...
                <header className="mt-10">
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                        <span className="mono text-sm text-amber">
                            {project.year}
                        </span>
                        <span className="label">{project.platform}</span>
                        {project.status && (
                            <span className="mono rounded-full border border-accent-dim px-2.5 py-0.5 text-[0.6875rem] text-accent">
                                {project.status}
                            </span>
                        )}
                    </div>
                        ...
                        <div className="mt-8 flex flex-wrap gap-3">
                            {project.links.map((link) => (
                                <a
                                    ...
                                    className="group inline-flex items-center gap-2 rounded-full border border-line bg-surface px-4 py-2 text-sm transition-colors hover:border-accent-dim hover:text-accent"
                                >
                                    {link.label}
                                    <ArrowUpRight
                                        size={15}
            ...
            {/* Spec table + overview */}
            <section className="grid gap-12 border-t border-line-soft py-14 lg:grid-cols-[minmax(0,1fr)_20rem] lg:gap-16">
                <Reveal>
            ...
            <section className="border-t border-line-soft py-14">
                ...
                >
                    <span>
                        ...
                        <span className="mt-2 block font-display text-3xl font-semibold transition-colors group-hover:text-accent sm:text-4xl">
                            {next.name}
                        </span>
                    </span>
                    <ArrowUpRight
